...
def parameters_to_vector(model, p=1.0):
    ...
    for name, para in model.named_parameters():
        ...
        if para.dtype.is_complex:
            real = para.real.detach()[local_mask]
            imag = para.imag.detach()[local_mask]
            vec_list.append(real.reshape(-1))
            vec_list.append(imag.reshape(-1))
        else:
...
def vector_to_param_dict(vec, meta):
    ...
    for (name, para0, mask) in meta:
        ...
        if para0.is_complex():
            ...
            imag_flat = vec[:, ptr:ptr + K] if batch_mode else vec[ptr:ptr + K]
            ptr += K

            if batch_mode:
                para = para0.unsqueeze(0).expand(batch_size, *para0.shape).clone().to(real_flat.dtype)
                para_flat_real = para.real.flatten(1)
                para_flat_imag = para.imag.flatten(1)
                idxs = mask_flat.nonzero(as_tuple=False).squeeze(-1)
                para_flat_real.scatter_(1, idxs.unsqueeze(0).expand(batch_size, -1), real_flat)
                para_flat_imag.scatter_(1, idxs.unsqueeze(0).expand(batch_size, -1), imag_flat)
                para = para_flat_real.view_as(para0) + 1j * para_flat_imag.view_as(para0)
            else:
                para_flat_real = para0.real.flatten().clone().to(real_flat.dtype)
                para_flat_imag = para0.imag.flatten().clone().to(real_flat.dtype)
                idxs = mask_flat.nonzero(as_tuple=False).squeeze(-1)
                para_flat_real.scatter_(0, idxs, real_flat)
                para_flat_imag.scatter_(0, idxs, imag_flat)
                para = para_flat_real.view_as(para0) + 1j * para_flat_imag.view_as(para0)
...
class LSR():
    ...
    def __init__(self, model, ax_train, u_train, ax_test, u_test, gridx_train):
        ...
        batch_size = 1000 # The product of batch_size and out_dim must be larger than rank_LSR.
    ...
    def LSR(self, rank=1000):
        ...
        with torch.no_grad():
            ...
            sigma = S.cpu().numpy()
            np.savetxt(f'MultiONet-Tanh-eigvalues-{rank}.dat', sigma, fmt='%.12e')  # 科学计数法，12位小数
