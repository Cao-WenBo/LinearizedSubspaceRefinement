    ...
    for case_prefix in case_prefix_list:
        ...
        for case_folder in case_folders:
            ...
            if os.path.isdir(case_folder_path):
                ...
                if not class_files: print(case_prefix+'无保存的类文件')

                # 提取文件名中的 epoch 数字
                epoch_numbers = []
                for file in class_files:
                    epoch = int(file.split('.')[0])  # 提取文件名中的数字
...
def jacobian_trans_batch(x, y, nodes, boundary=(0,0)):
    Xxi = diff2d_batch(x, nodes, dim=0, boundary=boundary[0])
    Xeta = diff2d_batch(x, nodes, dim=1, boundary=boundary[1])
    Yxi = diff2d_batch(y, nodes, dim=0, boundary=boundary[0])
    Yeta = diff2d_batch(y, nodes, dim=1, boundary=boundary[1])
    
    jac = torch.stack([Xxi, Yxi, Xeta, Yeta], dim=3).view(x.shape[0],x.shape[1],x.shape[2],2,2)
    jac_inv = torch.inverse(jac)
    return jac, jac_inv

def diff2d_jac_inv_batch(jac_inv, u, nodes, boundary=(0,0)):
    u_xi = diff2d(u, nodes, dim=0, boundary=boundary[0])
    u_eta = diff2d(u, nodes, dim=1, boundary=boundary[1])
    u_xieta = torch.stack([u_xi, u_eta], dim=2).view(u.shape[0],u.shape[1],2,1)
    u_xy = (jac_inv@u_xieta).squeeze()
...
def diff2d_xy_batch(x, y, u, nodes, boundary=(0,0)):
    jac, jac_inv = jacobian_trans_batch(x, y, nodes, boundary=boundary)
    u_xy = diff2d_jac_inv(jac_inv, u, boundary=boundary)
    return u_xy

def diff2d_batch(u, nodes, dim=0, order=1, boundary=0):
    # input: u is 3D, batch is the last dim
    # boundary=0: The boundaries use the same number of nodes
    # boundary=1: The boundaries use smaller number of nodes
    # boundary=2: Periodic boundary
    if dim == 1:
    ...
    if boundary == 0 or boundary == 1:
        ...
        for i in range(len(nodes)):
            if nodes[i] == 0: 
                ...
                if boundary == 0: nodes_bound = nodes - nodes[i]
                if boundary == 1: nodes_bound = nodes[nodes >= -ind]
                coeff_bound = coeff(nodes_bound, order).unsqueeze(2)
                nodes_ind = ind + nodes_bound
                du[ind] = du[ind] + (coeff_bound * u[nodes_ind]).sum(0)
            else:
                ind = nodes[i]-n + M-1
        ...
        u = torch.cat([u[m:], u, u[:n]]); (M, N, batch) = u.shape
        du = torch.zeros_like(u)
        for i in range(len(nodes)):
            du[-m:M-n] = du[-m:M-n] + coeff_u[i] * u[-m+nodes[i]:M-n+nodes[i]]
        du = du[-m:M-n]
