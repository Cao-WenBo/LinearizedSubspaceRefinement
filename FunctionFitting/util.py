...
#     obj.device = device
#     return obj
def load_class(file_name, device=False):
    with open(file_name, 'rb') as file:
        class Device_Unpickler(pickle.Unpickler):
            def find_class(self, module, name):
                # 处理 torch.storage 类中的对象，重载 _load_from_bytes 方法
                if module == 'torch.storage' and name == '_load_from_bytes':
                    # 如果 device 是 False，则保持原设备
                    if device:
                        return lambda b: torch.load(io.BytesIO(b), map_location=device)
                    else:
                        return lambda b: torch.load(io.BytesIO(b))
                else:
                    if module == 'TSONN2D_DCTN_v10':
                        module = '__main__'
                    return super().find_class(module, name)
    ...
    # 如果 device 不是 False，则将所有变量加载到指定设备
    if device:
        # 如果 obj 是 torch.Tensor 类型，直接移动到目标设备
        if isinstance(obj, torch.Tensor):
            obj = obj.to(device)
        elif isinstance(obj, dict):
            # 如果 obj 是字典类型（假设有多个 tensor）
            for key, value in obj.items():
                if isinstance(value, torch.Tensor):
                    obj[key] = value.to(device)
        elif hasattr(obj, 'device'):
            # 如果对象有 device 属性，则更新为目标设备
            obj.device = device
