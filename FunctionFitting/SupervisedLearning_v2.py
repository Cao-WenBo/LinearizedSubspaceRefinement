...
# save history model for analysis
...
class SupervisedLearning():
    ...
    def train(self, batch_size=32, epochs=500, lr=1e-3, val_ratio=0.2):

        N = self.X_data.shape[0]
        idx = torch.randperm(N)
        n_val = int(N * val_ratio)
        train_idx, val_idx = idx[n_val:], idx[:n_val]
        self.X_train, self.Y_train = self.X_data[train_idx], self.Y_data[train_idx]
        self.X_val, self.Y_val = self.X_data[val_idx], self.Y_data[val_idx]
        ...
        for epoch in range(1, epochs+1):
            ...
            data = {'params_vector_base':self.params_vector_base.cpu(), 'params_vector_delta':self.params_vector_delta.cpu()}
            torch.save(data, f'Result/{epoch-1}_model_LSR.pt')


            f0, f_linear = jvp(lambda params: self.forward(params, self.X_ref), (self.params_vector_base,), (self.params_vector_delta,))
            pred = f0 + f_linear
            test_loss_LSR = criterion(pred, self.Y_ref).item()
            ...
            for i in range(0, self.X_train.shape[0], batch_size):
                ...
                xb, yb = self.X_train[idx_batch], self.Y_train[idx_batch]
                optimizer.zero_grad()
                pred = self.model(xb)
            ...
            # test loss
            with torch.no_grad():
                test_pred = self.model(self.X_ref)
            ...
            self.log['test_loss_LSR'].append(test_loss_LSR)
            ...
            print(f'test_loss_LSR={test_loss_LSR:.4g}, time={self.log["time"][-1]:.2f}s')
...
if __name__ == '__main__':
    ...
    nn = SupervisedLearning(layers, device, folder_name='A200-v2-k1-mode1-pre')

    
    nn.train(batch_size=32, epochs=300)
